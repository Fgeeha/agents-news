...
Отклонённые рецензентом статьи сохраняются в
...
- «ОТКЛОНЕНО» уводит статью в `rejected/`, но не запускает переписывание.
  Дальше: цикл эксперт→рецензент до принятия (с лимитом итераций).
