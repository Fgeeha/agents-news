...
`make web` поднимает страницу, на которой конвейер разобран по шагам для
одной новости: слева — свежие новости из лент `config.yaml` (или любая
ссылка, вставленная в поле сверху), справа — карточка на каждого эксперта.

1. **Фильтр темы** — gate-модель отвечает, его ли это новость. Если «НЕТ»,
   в конвейере эксперт её пропустил бы; кнопка «Переписать всё равно»
   показывает, что получится, если фильтр проигнорировать.
2. **Эксперт пишет** — статья появляется в редактируемом поле.
3. **Рецензент проверяет** — вердикт и замечания. Текст статьи можно
   поправить руками (дописать «факт», которого нет в исходнике) и нажать
   «Проверить рецензентом» ещё раз — так видно, что именно ловит
   независимая модель.
4. **Доработка** — эксперт правит статью по замечаниям, рецензент
   проверяет повторно, как в боевом прогоне.
...
Эндпоинт LiteLLM по HTTPS с самоподписанным сертификатом — через
`SSL_CERT_FILE=<путь к PEM>`, код не меняется.
