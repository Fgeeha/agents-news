...
def serve(host: str = "0.0.0.0", port: int = 8080) -> ThreadingHTTPServer:
    ...
    logger.info("agents-news web: http://%s:%d/", host, port)
    return server


def main() -> None:
    """Точка входа: agents-news-web [--config config.yaml] [--port 8080]."""
    global config, llm
    parser = argparse.ArgumentParser(description="Web-интерфейс конвейера agents-news")
